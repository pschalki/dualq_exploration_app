...
TOPIC_OPTIONS = get_topic_options(ALL_DATA.get('participant_topic')) 
...
app.layout = dbc.Container([
    ...
    dbc.Tabs(id="main-tabs", active_tab='tab-cluster', children=[
        ...
        dbc.Tab(label='Cluster-Based Visuals', tab_id='tab-cluster', children=[
            ...
            dbc.Row([
                dbc.Col(dcc.Loading(dcc.Graph(id='cluster-scatter-plot')), width=8), 
                dbc.Col(dcc.Loading(html.Div(id='cluster-details-panel', children="Wählen Sie einen Cluster für Details.")), width=4) 
            ]),
            dbc.Row([
        ...
        dbc.Tab(label='Participant-Based Visuals', tab_id='tab-participant', children=[
            ...
            dbc.Row([
                # MODIFIED ID HERE
                dbc.Col(dcc.Loading(dcc.Graph(id='participant_scatter_GRAPH')), width=8), 
...
def get_plot_axes_data(df, emb_dict, selected_topic, sel_x_axis, sel_y_axis, id_col):
    x_coords, y_coords = [], []
    ...
    for _, row in df.iterrows():
        ...
        if selected_topic == "Gesamtes Interview":
            ...
            if isinstance(topic_emb_data, dict): 
        ...
        elif 'umap_x' in row and 'umap_y' in row and pd.notna(row['umap_x']) and pd.notna(row['umap_y']): 
...
def update_cluster_view(selected_topic, sel_x_axis, sel_y_axis, selected_cluster_id):
    ...
    current_df.dropna(subset=['plot_x', 'plot_y'], inplace=True) 
    if current_df.empty:
    ...
    if not triggered_id or triggered_id in ['cluster-topic-dropdown', 'cluster-x-axis-dropdown', 'cluster-y-axis-dropdown']:
        ...
        if selected_cluster_id and (triggered_id is None or triggered_id == 'selected-cluster-store'): 
             pass
...
@app.callback(
    # MODIFIED ID in Output
    Output('participant_scatter_GRAPH', 'figure'), 
...
def update_participant_view(selected_topic, coloring_var, sel_x_axis, sel_y_axis, selected_participant_id):
    ...
    if selected_topic != "Gesamtes Interview":
        ...
        if 'topic_summary_topic_specific' in current_df.columns:
             current_df.rename(columns={'topic_summary_topic_specific': 'topic_summary'}, inplace=True)
        elif 'topic_summary' not in current_df.columns: 
    ...
    current_df['plot_x'] = x_coords
    current_df['plot_y'] = y_coords
    current_df.dropna(subset=['plot_x', 'plot_y', 'interview_id'], inplace=True) 
    if current_df.empty:
...
@app.callback(
    ...
    # MODIFIED ID in Output
    Output('participant_scatter_GRAPH', 'figure', allow_duplicate=True),
    # MODIFIED ID in Input
    Input('participant_scatter_GRAPH', 'clickData'),
    State('participant-topic-dropdown', 'value'),
    State('selected-participant-store', 'data'),
    # MODIFIED ID in State
    State('participant_scatter_GRAPH', 'figure'),
    prevent_initial_call=True
)
def display_participant_click_data(clickData, selected_topic, current_selected_id, current_fig_state):
    ...
    if clicked_participant_id == current_selected_id:
        ...
        elif not participant_info.empty: 
            if 'overall_summary' in participant_info.columns and 'topic_summary' not in participant_info.columns:
                 participant_info.loc[:, 'topic_summary'] = participant_info['overall_summary'] 
    ...
    if current_fig_state:
        ...
        for trace in updated_fig.data:
            if hasattr(trace, 'customdata') and trace.customdata is not None:
                opacities = []
                for cd_point_tuple in trace.customdata: 
